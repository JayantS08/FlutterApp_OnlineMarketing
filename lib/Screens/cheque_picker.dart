...
class ChequePick extends StatelessWidget {
  String id;
  ChequePick(this.id);
  @override
  Widget build(BuildContext context) {
    return Container(
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Upload a Cancelled Cheque'),
...
class ChequeInput extends StatefulWidget {
  String id;
  ChequeInput(this.id);
  @override
  State<StatefulWidget> createState() {
    return _ChequeInput(id);
  }
}
...
class _ChequeInput extends State<ChequeInput> {
  String id;
  _ChequeInput(this.id);
        ...
        Container(child: RaisedButton(child: Text('Ok',style: TextStyle(fontSize: 15.0),),elevation: 5,
          ...
          onPressed:  () async{
            if(_uploadedFileURL!=null)
            {
              try {
                print('registered');
                DocumentReference res = await _firestore.collection('Cancelled Cheque').add({
                  'UID': id,
      ...
      actions: <Widget>[
        Container(child: RaisedButton(child: Text('Ok',style: TextStyle(fontSize: 15.0),),elevation: 5,
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => ChequePick(id)),
            );
          },
        ),alignment: Alignment.center,
        )
